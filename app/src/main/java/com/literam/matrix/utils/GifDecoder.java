...
import com.literam.matrix.entity.GifFrameBean;
...
public class GifDecoder {
    ...
    private static final int STATUS_FORMAT_ERROR = 1;//格式错误
    private static final int STATUS_OPEN_ERROR = 2;//打开图片失败

    private int status;//解码状态
    private InputStream in;

    protected int width;//完整的GIF图像宽度
    protected int height;//完整的GIF图像高度
    private boolean gctFlag;//是否使用了全局颜色列表
    private int gctSize; //全局颜色列表大小
    private int loopCount = 1; // iterations; 0 = repeat forever

    private int[] gct; //全局颜色列表
    private int[] lct; //局部颜色列表
    private int[] act; //当前使用的颜色列表

    private int bgIndex; //背景颜色索引
    private int bgColor; //背景颜色
    private int lastBgColor; // previous bg color
    private int pixelAspect; //像素宽高比(Pixel Aspect Radio)

    private boolean lctFlag;//局部颜色列表标志(Local Color Table Flag)
    private boolean interlace;//交织标志(Interlace Flag)
    private int lctSize;//局部颜色列表大小(Size of Local Color Table)

    private int ix, iy, iw, ih; //当前帧图像的xy偏移量及宽高
    private int lrx, lry, lrw, lrh;
    private Bitmap image; // current frame
    private Bitmap lastImage; // previous frame
    private int frameIndex = 0;

    private boolean isScale=true;//是否缩放
    private int scaleSize = 16;//缩放等级
    ...
    private int blockSize = 0; //扩展块大小

    // last graphic control extension info
    private int dispose = 0;
    // 0=no action; 1=leave in place; 2=restore to bg; 3=restore to prev
    private int lastDispose = 0;
    private boolean transparency = false;//是否使用透明色
    private int delay = 0;//延迟时间(毫秒)
    private int transIndex;//透明色索引
    ...
    private Vector<GifFrameBean> frames;// 存放各帧对象的数组
    ...
    public GifFrameBean getFrameBean(int n){
        if ((n >= 0) && (n < frameCount)) {
    ...
    private void setPixels() {
        ...
        //image = ScaleAndGray(Bitmap.createBitmap(dest, width, height, Bitmap.Config.RGB_565));
    }

    public void setScale(boolean isScale,int scaleLevel){
        this.isScale = isScale;
        this.scaleSize = scaleLevel;
    }

    private Bitmap ScaleAndGray(Bitmap origin){
    ...
    public Vector<GifFrameBean> getFrameList(){
        return frames;
    }
    ...
    public int read(InputStream is) {
        ...
        /*for (int i = 0; i < frameCount; i++) {
            Bitmap image = frames.elementAt(i).image;
            frames.elementAt(i).image = ScaleAndGray(image);
        }*/
                ...
                case 0x21: //扩展块标识，固定值0x21
                    ...
                    switch (code) {
                        ...
                        case 0xff: //应用程序扩展块标识(Application Extension Label)，固定值0xFF
                            readBlock();
                            if (getBlockApp().equals("NETSCAPE2.0")) {
                                readNetscapeExt();
                            } else {
                                skip(); // don't care
                            }
    ...
    private String getBlockApp(){
        if(block.length == 0) return "";
        byte[] app = new byte[11];
        System.arraycopy(block, 0, app, 0, 11);
    ...
    private void readImage() {
        ...
        frames.addElement(new GifFrameBean(image, delay,frameCount));//添加到帧图集合
        // list
        if (transparency) {
